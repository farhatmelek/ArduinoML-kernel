defmodule ArduinoML.CodeProducer do
  ...
  defp action_statement(%ArduinoML.Exception{pause_time: pause_time, error_number: error_number,actuator: %{label: label, type: type},signal: _}) do
    blinking_actions=Enum.map(1..error_number, fn _   ->
      ...
      #{write_function(type)}(#{brick(label)}, #{signal(:high)});
       delay(1000); # Wait for 1000 millisecond(s) to notice the blinking.
      #{write_function(type)}(#{brick(label)}, #{signal(:low)});
      """
    end)

    Enum.join(blinking_actions, "\n") <> "\n" <> "#delay(#{pause_time});"
  ...
  defp transitions_declaration(application, [%{to: to, on: condition} | others], is_first) do
    condition = condition
    transformed_condition=transform_condition(condition)
    "#{condition_keyword(is_first)} (#{transformed_condition}) {\n" <>
  ...
  defp transform_condition(%ArduinoML.MultipleCondition{operator: operator, conditions: sub_conditions}) do
      nested=Enum.map(sub_conditions,&transform_condition(&1))
      Enum.join(nested," #{operator_to_string(operator)}" )
  end
  defp operator_to_string(:and), do: "&&"
  defp operator_to_string(:or), do: "||"
  defp operator_to_string(_), do: raise "Unknown operator"
  ...
  defp signal(label) when label in [:blink], do: :high |> Atom.to_string |> String.upcase
